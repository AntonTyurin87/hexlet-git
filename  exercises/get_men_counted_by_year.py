...
def get_men_counted_by_year(users):
    ...
    for i in users:
        if i.get('gender') == 'male':
            men_list.append(date.fromisoformat(i.get('birthday')).year)
    
    return dict(Counter(men_list))
...
Реализуйте функцию get_men_counted_by_year(), которая принимает на вход список пользователей и возвращает словарь, 
в котором ключ — это год рождения, а значение — количество мужчин, родившихся в этот год.

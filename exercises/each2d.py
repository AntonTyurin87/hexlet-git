matrix = [[1, "Foo", 100], [False, 10, None]]

def is_int(x):
    return isinstance(x, int)

def each2d(is_int, matrix):
    return all((all((is_int(x) for x in j)) for j in matrix))

def some2d(is_int, matrix):
    return any((any((is_int(x) for x in j)) for j in matrix))

def sum2d(is_int, matrix):
    return sum((sum((x for x in j if is_int(x))) for j in matrix))

print(each2d(is_int, matrix))

print(some2d(is_int, matrix))

print(sum2d(is_int, matrix))

'''
[.. for ... in [... for ... in ...] ...] 

Вам предстоит реализовать три функции, каждая из которых будет работать с двухмерной матрицей, то есть со списком списков (итератором итераторов):
...
в противном случае (если ни один элемент проверку не прошел) нужно вернуть False.

sum2d(test, matrix) должна возвращать сумму всех элементов матрицы matrix, удовлетворяющих предикату test.

Внимание, первые две функции не должны выполнять лишней работы: обход матрицы должен прерываться, как только результат становится ясен.

from solution import each2d, some2d, sum2d
def is_int(x):
    return isinstance(x, int)
